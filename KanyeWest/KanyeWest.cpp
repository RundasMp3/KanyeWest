...
int main(int argc, char** argv)
{
	...
	long filend;
	fopen_s(&fp, "C:\\Users\\beto_\\Downloads\\IsThisRegresion.ye", "rb");
	...
	//st_stack stack[17];
	unsigned char value1 = (buffer[7]);
	unsigned char value2 = (buffer[8]);

	int TSC = (int)((value1 << 8) | value2);
	value1 = (buffer[9]);
	value2 = (buffer[10]);
	int TSD = (int)((value1 << 8) | value2);
	//stack->dato = ((buffer[9] << 8) | buffer[10]);
	sc = (unsigned char*)malloc(TSC * sizeof(char*));
	sd = (unsigned char*)malloc(TSD * sizeof(char*));
...
void LeerYEscribirIntenDir(unsigned char* sd, int dir)
{
...
void LeerYEscribirDoubleenDir(unsigned char* sd, int dir)
{
...
void LeerYEscribirCharenDir(unsigned char* sd, int dir)
{
	unsigned char n;
	scanf_s("%c", &n);
	//Get each byte value from int.
...
void EscribirDoubleenDir(double n, unsigned char* sd, int dir)
{
	unsigned char bytes[8];
	union {
		double doble;
...
void EscribirCharenDir(unsigned char n, unsigned char* sd, int dir)
{
	//Get each byte value from int.
...
void EscribirStringenDir(char* n, unsigned char* sd, int dir)
{
...
void SUBVARS(st_stack* stack)
{
	...
	case 'i':
		...
		{
			res.tipo = 'd';
			res.dato.doble =  aux.dato.doble - (double)nodo.dato.entero;
			nodo = res;
		}
		else
		{
			aux.dato.entero -= nodo.dato.entero;
			nodo = aux;
		}
	...
	case 'd':
		...
		{
			res.tipo = 'd';
			res.dato.doble = (double)aux.dato.entero - nodo.dato.doble;
			nodo = res;
		}
		else
		{
			aux.dato.doble -= nodo.dato.doble;
			nodo = aux;
		}
	...
	case 'c':
		...
		aux.dato.caracter -= nodo.dato.caracter;

		break;
	case 's':
		//aux.dato.apuntador -= nodo.dato.apuntador;
		break;
...
void MULTVARS(st_stack* stack)
{
	...
	case 'i':
		...
		{
			res.tipo = 'd';
			res.dato.doble = (double)nodo.dato.entero * aux.dato.doble;
			nodo = res;
		}
		else
		{
...
void INCVARD(st_stack* stack)
{
	st_node nodo = pop(stack);
	nodo.dato.doble++;
	//printf("SUMVALUE:\t%lf", nodo.dato.doble);
	push(stack, nodo);
}
...
void CMPVARLE(st_stack* stack)
{
	...
	case 'c':
		...
		break;
	case 's':
		if (strcmp(node1.dato.apuntador, node2.dato.apuntador) > 0)
		{
			//result = true;
			result.dato.caracter = true;
			push(stack, result);
...
void PRTVAR(int dir, unsigned char* sd, char type)
{
	...
	case 'd':
	{
		...
		for (int i = 0; i < 8; i++)
		{
			translate.bytes[i] = sd[dir++];
		}
		printf("%lf", translate.num);
	}
		break;
	case 'c':
		printf("%c", sd[dir]);
...
void running(unsigned char* sc, int pc, unsigned char* sd, st_stack* stack)
{
	int dir = 0;
	int valuei = 0;
	st_node nodo;
	...
	while (sc[pc] != EFE)
	{
		...
		if (sc[pc] != EFE)
		{
			...
			case POPI:
				...
				break;
			case POPD:
				if (top(stack).tipo != 'd' && top(stack).tipo != 'i')
				{
				...
				if (nodo.tipo == 'i')
				{
					EscribirDoubleenDir((double)nodo.dato.entero, sd, dir);
				}
				else
				{
			...
			case POPS:
				...
				pc += 2;
				nodo = pop(stack);
				EscribirStringenDir(nodo.dato.apuntador, sd, dir);
				break;
			case POPB:
				if (top(stack).tipo != 'c')
				{
			...
			case BRNCHC:
			{
				unsigned char value1 = (sc[pc + 1]);
				unsigned char value2 = (sc[pc + 2]);

				dir = (unsigned int)(value1 << 8) | value2;
				pc += 2;
				nodo = pop(stack);
